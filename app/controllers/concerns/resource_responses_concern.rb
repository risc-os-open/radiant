...
module ResourceResponsesConcern
  extend ActiveSupport::Concern

  # ============================================================================
  # Support classes held within this concern's namespace
  # ============================================================================
  ...
  included do
    class_attribute :declared_responses, instance_accessor: false, instance_writer: true
  end

  class_methods do
    def declare_responses
      self.declared_responses ||= Collector.new
      yield self.declared_responses if block_given?
      self.declared_responses
    end
  end

  def response_for(action)
    responses = self.class.declared_responses.send(action)
    respond_to do |wants|
