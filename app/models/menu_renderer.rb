module MenuRenderer
  ...
  def menu_list
    view.tag.ul(class: 'menu', id: "allowed_children_#{id}") do
      view.safe_join(menu_items, '')
    end
  end

  def remove_link
    view.link_to(
      view.image('minus') + ' ' + I18n.t('remove'),
      view.remove_admin_page_url(self),
      class: 'action'
    )
  end
  ...
  def disabled_add_child_link
    tag.span(class: 'action disabled') do
      view.image('plus_disabled') + ' ' + I18n.t('add_child')
    end
  end

  def add_child_link
    view.link_to(
      view.image('plus') + ' ' + I18n.t('add_child'),
      view.new_admin_page_child_path(self, page_class: default_child.name),
      class: 'action'
    )
  end
  ...
  private
    ...
    def menu_item(child_class)
      view.tag.li(menu_link(child_class))
    end
