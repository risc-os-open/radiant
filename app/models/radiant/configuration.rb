module Radiant
  ...
  # The Radiant::Configuration model class is stored in the database (and cached) but emulates a hash
  # with simple bracket methods that allow you to get and set values like so:
  #
  #   Radiant.configuration['setting.name'] = 'value'
  #   Radiant.configuration['setting.name'] #=> "value"
  #
  ...
  # declared by calling Radiant::Configuration#define:
  #
  ...
  class Configuration < ::ApplicationRecord
    ...
    class << self
      def [](key)
        if table_exists?
          unless Radiant::Configuration.cache_file_exists?
            Radiant::Configuration.ensure_cache_file
            Radiant::Configuration.initialize_cache
          end
          Radiant::Configuration.initialize_cache if Radiant::Configuration.stale_cache?
      ...
      def stale_cache?
        return true unless Radiant::Configuration.cache_file_exists?
      ...
      #
      #   choose_layout = Radiant::Configuration::Definition.new(:select_from => lambda {Layout.all.map{|l| [l.name, l.d]}})
      ...
      def define(key, options={})
        ...
        if options.is_a? Radiant::Configuration::Definition
          definition = options
        else
...
2. #{definitions[key].definer}
        ...
        definition ||= Radiant::Configuration::Definition.new(options.merge(:definer => called_from))
      ...
      def definition_for(key)
        definitions[key] ||= Radiant::Configuration::Definition.new(:empty => true)
      end
